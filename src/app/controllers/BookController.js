...
class BookController {
  ...
  async indexNotUser(req, res) {
    ...
    const books = await Book.findAndCountAll({
      where: category
        ? { user_id: { [Op.ne]: user_id }, is_active: true, category }
        : { user_id: { [Op.ne]: user_id }, is_active: true },
  ...
  async indexOne(req, res) {
    ...
    if (book === null) return res.status(404).json({ id: id });
  ...
  async update(req, res) {
    ...
    let invalidKeys = [];
    for (let key in body) {
      if (!(key in dataValues)) invalidKeys.push(key);
    }
